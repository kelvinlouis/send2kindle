...
/**
 * Convert multiple articles into a single EPUB with chapters.
 *
 * @param {Object} options
 * @param {Array<{title: string, htmlContent: string}>} options.chapters - Chapters to include
 * @param {string} [options.title="Book"] - Book title
 * @param {string|null} [options.author=null] - Book author
 * @param {boolean} [options.debugMode=false] - Save files to cwd instead of tmpdir
 * @returns {string} Path to the created EPUB file
 */
export function convertBookToEpub({ chapters, title = 'Book', author = null, debugMode = false }) {
  console.log(`\u{1F4DA} Converting ${chapters.length} chapters to EPUB...`);
...
`;
  ...
  const chapterHtml = chapters
    .map(
      (ch) => `<h1>${ch.title}</h1>
${replaceYouTubeEmbeds(ch.htmlContent)}`,
    )
    .join('\n');
